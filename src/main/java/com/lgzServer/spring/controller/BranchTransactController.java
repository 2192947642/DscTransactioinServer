...
public class BranchTransactController {
     ...
     public void noticeBranchTransaction(@RequestBody BranchTransaction branchTransaction){
         ...
         if(globalTransaction.getStatus()==GlobalStatus.wait){
             if(branchTransaction.getStatus()== LocalStatus.success){//如果是执行成功了
                     boolean isWait=false;
                     boolean isFail=false;
                     for(BranchTransaction branchTransaction1:lists){
                         if(branchTransaction1.getStatus()== LocalStatus.wait){
                             isWait=true;
                         }
                         if(branchTransaction1.getStatus()== LocalStatus.fail||branchTransaction1.getStatus()== LocalStatus.rollback){
                             isFail=true;
                         }
                     }
                     if(isFail){
                         ...
                         globalTransactionMapper.updateGlobalTransaction(branchTransaction.getGlobalId(),GlobalStatus.success.toString());
    ...
    public Result<BranchTransaction> joinBranchTransaction(@RequestBody BranchTransaction branchTransaction){
        if(branchTransaction.getBranchId() ==null){
            branchTransaction.setBranchId(BranchTransaction.generateLocalId());
        }
